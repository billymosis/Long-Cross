                    ...
                    using (Transaction tr = doc.TransactionManager.StartTransaction())
                    {
                        ...
                        foreach (var item in btr)
                        {
                            try
                            {
                                ...
                                {
                                    var c = item.GetObject(OpenMode.ForWrite) as DBText;
                                    if (c.TextString.Substring(0, 1) == "+")
                                    {
                                        var Elev = double.Parse(c.TextString.Substring(1)) + dob.Value;
                                        c.TextString = "+" + Math.Round(Elev, 2).ToString();
                                    }
                                }
                                if (item.ObjectClass.DxfName == "MTEXT")
                                {
                                    var c = item.GetObject(OpenMode.ForWrite) as MText;
                                    var cb = c.Text;
                                    if (c.Text.Substring(0, 1) == "+")
                                    {
                                        var Elev = double.Parse(c.Text.Substring(1)) + dob.Value;
